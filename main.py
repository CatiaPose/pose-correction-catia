...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*", "http://localhost:10016"],  # Change this to the actual frontend URL in production
...
async def get_pose_correction(mediaType: str = Form(...), mediaFile: UploadFile = File(...), options: str = Form(...)):
    ...
    media_file_path = os.path.join("media/temp.jpeg")
    ...
    if mediaType == "image":
        ...
        with mp_pose.Pose(
                ...
                min_tracking_confidence=0.3) as pose:
                    ...
                    image = cv2.imread("media/temp.jpeg")
    ...
    if mediaType == "webcam":
        ...
        with mp_pose.Pose(
                ...
                while cap.isOpened() and window_open:
                    ...
                    # To improve performance, optionally mark the image as not writeable to
                    # pass by reference.
                    # Flip the image horizontally (uncomment the line below)
...
async def get_pose_correction(mediaType: str = Form(...), mediaFile: UploadFile = File(...), options: str = Form(...)):
         # Save the uploaded file
    media_file_path = os.path.join("media/temp.mp4")
    with open (media_file_path, "wb") as buffer:
        shutil.copyfileobj(mediaFile.file, buffer)
    
    # fourcc = cv2.VideoWriter_fourcc(*'H264')
    # out = cv2.VideoWriter('E:/candice/output.mp4', fourcc, 20.0, (600, 600))
